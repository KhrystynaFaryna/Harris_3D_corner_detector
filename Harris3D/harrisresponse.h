...
class HarrisResponse
{
    ...
    H_vertices* vertex;
    H_vertices* harrisvertex;

    std::set<int> candidate_v;
    H_faces* face;
    ...
    void computeHarrisResponse(int rings,double H=0.04);
    void findcandidates(std::string filename, std::string filecan);
    bool comp_resp(int, int);
    void updateadjpts();
    double calcmax(float a[], int n );
    ~HarrisResponse();
};
